...
## 2026-09-14 — End-of-day handoff

- Todd ended implementation work for the day and requested GitHub synchronization through GitHub Desktop. Tomorrow's priority is the working integration: reliable inventory synchronization and online eligibility/removal behavior. Do not restart completed copy-protection/zero-stock tests or divert into legacy-content redevelopment.
- Main project remote is public. Raw staging CSV and private credentials remain local/excluded; reviewed findings, implementation source, release packages and project notes are the synchronization scope. Inaccessible old generated preview files must not be committed as deletions.
- Nested published-site repository uses a separate Sites-hosted remote, not GitHub. It has one existing local ignore-rule commit; remote authentication failed during verification. Do not change credentials or remotes automatically. GitHub sync is not a WordPress database/media/server backup or a website deployment.

## 2026-09-14 — Spreadsheet, live Revel and complete staging catalog reconciled

- Todd clarified Mark's25-row spreadsheet does not prove changes were entered into Revel. Reopened workbook read-only, matched25 unique barcodes to recorded RevelIDs; normalized names/prices/categories agree with the previous map. Two SKU discrepancies are Energy and Ear Clear; barcode remains the key. Fresh Revel NAC/Ear Clear checks confirm identities and empty descriptions, with no source writes.
- Recovered in-app export after timeouts and downloaded native staging catalog.320 CSV rows reconcile to306 products plus14 variations, not new imports.245 published/61 parent drafts. Exact pilot SKU join finds legacy EnergyID1108 only;24 lack exact SKU matches. Found legacy NACID1167 with empty SKU, existing copy and image; needs formulation/pack verification before linking, not automatic duplication or merge.
- Corrected missing-content assumption:60 of61 legacy parent drafts contain descriptions, and46 export rows contain image URLs. Energy includes historical directions/warnings/Supplement Facts. Legacy NAC text explicitly lists1 tablet and1,000mg, strengthening the identity candidate; pack count/current barcode still require verification. These are reuse candidates requiring current identity/label verification, not automatically approved content. Recent245 imports have244 empty long descriptions, no images,238 blank stock values,2 explicit zero and5 positive. All export weights blank.
- Saved export in client-inputs/phase-two, added reusable read-only comparison script, updated canonical map and placed unsent support draft on HOLD. Next work is legacy identity/content reconciliation and reliable stock initialization, not repeating completed protection/zero-stock tests or assuming Mark changed Revel. No products, quantities, source flags, schedules, live site or paid services changed. No email sent.

## 2026-09-09 - Header navigation mockup revised

- Created Header + Footer Ecommerce Add-On v1.6 without overwriting the previously approved v1.5 file.
- Moved Shop Fullscript out of the In-Store Products submenu and placed it directly on the main navigation bar, matching Todd's requested current-site structure.
- Updated the Phase Two review hub, version register, and mockup inventory. The new exact version is awaiting Todd review before it is presented to Rebekah.
...
## 2026-09-08 — Kosmos Partner Connect reply reviewed and response drafted

- Reviewed Kosmos's unread response to support ticket `#461376`. Kosmos said Rebekah's client account must sign up for Revel Partner Connect to obtain the API key and secret, and asked Blue Nova for the client's Revel URL so Kosmos can contact Revel API support.
- Confirmed the exact Revel account address from the project record as `https://rebekahs.revelup.com`; no address was guessed and no credential was exposed.
- Prepared an unsent Gmail reply draft to Kosmos that supplies the verified Revel URL, asks for the exact Partner Connect approval/client step, and repeats the approved safety boundary: manual one-way Revel-to-staging-WooCommerce product import only, all schedules off, all WooCommerce-to-Revel writeback off, and no Revel changes. At Todd's request, updated the same saved draft to require written confirmation that Partner Connect enrollment, the API credentials, and Kosmos/Revel API-support coordination are free; if any fee, contract, subscription change, paid support, or other charge could apply, Kosmos must stop and provide the exact amount and terms before proceeding. The draft expressly authorizes no charge.
- No email was sent. No Partner Connect enrollment, Revel API credential, Revel authorization, Kosmos Action, schedule, product import, synchronization, payment, shipping, live-site, or staging-to-live change occurred.
- Next step: Todd reviews and explicitly approves sending the saved reply draft. After Kosmos/Revel provides the exact Partner Connect instructions and permissions, discuss that specific authorization with Todd before taking any Revel-side action.

## 2026-09-08 — Client progress and schedule update drafted

- Prepared an unsent Gmail draft to Rebekah summarizing the completed Cloudways staging store, approved ecommerce implementation, recoverable legacy-product quarantine, 25-product audit, representative-product selection, and staging WooCommerce connection to Kosmos with zero Actions or schedules enabled.
- The draft explains that the live Revel system remains unchanged and that the next integration gate is written Kosmos/Revel Partner Connect guidance plus API credentials. It states that written confirmation of no Partner Connect/API/support-coordination charge is required and that nothing billable will be approved without prior discussion.
- The draft distinguishes later blockers: Clover/Fiserv access is needed for payment testing, USPS developer access is needed for live-rate testing, and remaining store-policy decisions need client approval before launch. These do not block the controlled one-product staging test.
- The draft tells Rebekah that Blue Nova will work until noon Wednesday, September 9, 2026, pause project work after noon, and resume next week. It also says she does not need to change Revel now and that Blue Nova will contact her if Revel sends a specific Partner Connect approval or enrollment request.
- No email was sent and no Revel, Kosmos, payment, shipping, live-site, or staging configuration was changed.

## 2026-09-08 — Remaining Rebekah requests reconciled before follow-up draft

- Reconciled Rebekah and Mark's Phase Two email threads, the USPS plugin thread, the current master checklist, Policy Review Packet v1.2, staging QA, integration notes, and the September 8 Gmail status.
- Confirmed Blue Nova should not ask again for the Clarkston fulfillment location/address, two-day handling period, continental-U.S.-only region, no-PO-box rule, USPS carrier/account number, free Octolize plugin choice, notice recipients, tax treatment, operating defaults, 25-product export, Revel access, Kosmos access, or approved storefront designs. Clover/Fiserv was excluded from this review at Todd's request.
- Confirmed the free Octolize live-rate plugin still requires USPS Developer Portal Consumer Key/Secret credentials; the existing USPS account number alone is not API access. Rebekah does not need to buy a plugin. Blue Nova will handle the technical setup and request owner verification only if USPS requires it.
- The remaining direct client questions are: approve `REBEKAH'S Energy 90 Caps` as the controlled first product and confirm its Clarkston price/inventory; identify any pilot item that cannot be mailed or needs special handling; approve the online-return destination/contact rule, cancellation cutoff, damaged/missing-item reporting rule, delivered-but-not-received handling, return-postage responsibility, final-sale restrictions, and customer-facing ecommerce support email.
- Rebekah should also forward any Revel Partner Connect enrollment or approval notice before accepting terms or a charge. Product-weight field placement and the consolidated missing product-content list remain Blue Nova follow-ups after the one-product proof, not current client homework.
- Gmail now shows that Todd sent both the Kosmos ticket reply and the earlier `Phase Two online store update and schedule` message on September 8. No additional Gmail draft was created during this reconciliation because Todd requested that the proposed follow-up be written in chat for review first.

## 2026-09-08 — One-product proof approach clarified

- Todd asked whether Rebekah's team should create an editable test product in Revel. Decided against requesting a fabricated Revel product because Revel is the live production POS, a synthetic record may not reproduce the real 25-product data, and creating or editing it would add avoidable production risk.
- The controlled proof should use the existing representative item `REBEKAH'S Energy 90 Caps` as the unchanged Revel source. Import it manually into protected WooCommerce staging, document the actual incoming fields, then make test edits only to the staging WooCommerce copy and rerun the one-way import to identify which fields Kosmos overwrites.
- If a Revel-originated update can only be proven by changing a source value, stop first, identify the exact harmless field and rollback value, discuss it with Todd, and obtain separate action-time authorization. Any approved source edit should be made by Mark/Rebekah's Revel team rather than inferred from general integration approval.
- Revised the proposed client question so Rebekah is asked to choose the public customer-service email for online-order questions and returns; do not assume the already-approved internal order-notification addresses are also the public support address.
- After Todd reviewed the wording in chat, saved the follow-up in Gmail Drafts with subject `Phase Two: a few remaining decisions`, addressed to Rebekah and copying Mark as previously authorized for Phase Two. The draft asks for the representative-product confirmation, shipping exceptions, focused ecommerce policy decisions, and the public customer-support email; it explains that no fabricated Revel product or USPS plugin purchase is needed and requests that any Revel Partner Connect approval be forwarded before terms or charges are accepted.
- The follow-up remains unsent. No Revel, Kosmos, USPS, WooCommerce, live-site, payment, or staging configuration changed while preparing it.

## 2026-09-08 — August GSC performance emails reviewed

- Google sent separate monthly summaries for the domain property `rebekahspureliving.com` and URL-prefix property `https://rebekahspureliving.com/`. They reported 1.27K clicks / 12.1K impressions and 853 clicks / 10.9K impressions respectively, with 101 pages receiving first impressions. GBP-tagged Lapeer, Grand Blanc, and Lake Orion location URLs drove the largest gains.
- Refreshed exact-domain cloud evidence covers August 9–September 5 and shows 1,104 clicks, 11,203 impressions, and 9.9% CTR: 32 fewer clicks with 58 more impressions. The legacy `http://www.rebekahspureliving.com/home/` path correctly redirects to the clean homepage.
- The queue flags `/shop/` as live but `noindex`; that remains appropriate while the controlled ecommerce build is not launched. Status: **monitor / no indexing action**. No site or Google setting was changed.

## 2026-09-08 — Kosmos contacted Revel; no Todd action currently required

- Reviewed Kosmos's newest full-thread response on ticket `#461379` / original request `#461376`. James confirmed that Kosmos contacted Revel and will update Blue Nova after Revel responds. Revel will also contact Rebekah directly to confirm the request.
- Todd does not need to reply to Kosmos now. Rebekah has already been told to forward any Partner Connect enrollment or approval notice before accepting terms or charges.
- Kosmos has not yet confirmed that Partner Connect/API coordination is free and has not yet confirmed the manual one-way Revel-to-staging-WooCommerce test with schedules and all WooCommerce-to-Revel writeback disabled. Those safeguards remain open and must be confirmed before credentials are connected.
- No email was drafted or sent during this review. No Revel authorization, credential, Kosmos Action, schedule, synchronization, product import, charge, payment, shipping, live-site, or staging configuration changed.

## 2026-09-08 — Rebekah Partner Connect heads-up wording prepared

- Prepared concise client-facing wording in chat to warn Rebekah that Revel may contact her directly to confirm Kosmos Partner Connect/API access.
- The proposed message asks her to forward the Revel email before accepting anything and not to approve a fee, subscription, contract, paid support, or broader Revel permission without first checking with Blue Nova.
- No Gmail draft was created and no email was sent. No Revel, Kosmos, website, payment, shipping, or staging setting changed.

## 2026-09-08 — Partner Connect heads-up wording corrected

- Clarified that disabling all WooCommerce-to-Revel writeback applies to the initial one-product import test, not automatically to the final live architecture. Revel-to-WooCommerce product and inventory updates can remain one-way, but a live store will likely need a narrowly scoped WooCommerce-order-to-Revel action so online sales reach the POS and Clarkston inventory remains accurate. The exact order, customer, and inventory effects must be proven on staging and separately approved before activation.
- Revised the proposed fee wording from an absolute prohibition to a review-first instruction. Rebekah should forward any Revel request and speak with Blue Nova before accepting a fee, contract, subscription change, paid support, or broad permission, because a legitimate disclosed charge may exist but must not be accepted unknowingly.
- No Gmail draft was created or changed and no email was sent. No Revel, Kosmos, WooCommerce, payment, shipping, live-site, or staging setting changed.

## 2026-09-08 — Partner Connect heads-up sent to Rebekah

- With Todd's explicit approval, sent the exact reviewed email `Heads-up: Revel may contact you about Partner Connect` to Rebekah at the established client address.
- The message asks Rebekah to forward any Revel email, agreement, or approval before accepting it; to consult Blue Nova before agreeing to fees, paid support, subscription changes, contracts, or broad permissions; and clarifies that writeback is disabled for the initial one-product staging test but a narrowly scoped order/inventory flow may later be tested and separately approved for launch.
- Gmail verification shows the message in Sent. No Revel authorization, credential, Kosmos Action, schedule, synchronization, product import, charge, payment, shipping, live-site, or staging configuration changed.

## 2026-09-08 — Mark changed pilot product and approved operating-policy recommendations

- Reviewed Mark Cobleigh's complete reply in the `Phase Two: a few remaining decisions` thread. Rebekah was copied on his response.
- Mark replaced the earlier first-product candidate with the existing `REBEKAH'S NAC 1000mg` because it was freshly restocked. He said the store may discontinue `REBEKAH'S Energy 90 Caps`, so it must not be used for the controlled proof. This selects the candidate only; it does not authorize an import or a Revel edit.
- Mark designated `clarkstonpurchaser@rebekahspureliving.com` for customer-facing/live online-order communications. He requested that controlled test messages use `lapeerpurchaser@rebekahspureliving.com` so Clarkston staff are not confused by staging traffic. No staging email setting was changed; outbound mail remains blocked until a separately authorized delivery test.
- Mark agreed with all recommended return, cancellation, damage/missing-item, delivered-package, postage-responsibility, and opened/final-sale rules. Updated Policy Review Packet v1.2 and the master checklist to record those client decisions. Qualified legal review and publication remain separate gates.
- One client clarification remains open: Mark did not explicitly state whether none of the 25 selected products requires special handling or cannot be mailed.
- Created a reply-all Gmail draft in Mark's original thread, addressed to Mark and copying Rebekah. It acknowledges the NAC selection and email routing, confirms the approved policy record, and asks directly whether any of the 25 products has a shipping restriction or cannot be mailed.
- After Todd reviewed and explicitly approved the revised wording, sent that exact reply. Gmail verification shows it in Sent in the original thread. No Revel, Kosmos, WooCommerce, USPS, payment, live-site, or staging setting changed.

## 2026-09-09 — Mark, Rebekah, Kosmos, and Revel email reconciliation

- Reviewed the latest complete Gmail threads from Mark, Rebekah, Kosmos, and Revel. Todd did not receive a separate direct Revel email; the Revel response was forwarded by Rebekah, and the ongoing Kosmos/Revel coordination thread includes Todd by copy.
- Mark directly confirmed that none of the 25 selected products has a shipping restriction. Closed the remaining pilot shipping-exception question in the master checklist and Policy Review Packet v1.2. The selected first-test item remains the existing `REBEKAH'S NAC 1000mg`; the possibly discontinued Energy product remains excluded.
- Revel case **#03058403** requires **Partner Connect Unlimited at $25/month per location** before Revel will release API credentials directly to Kosmos. Revel also requests signed documents/order form and client authorization to give Kosmos production credentials for the Revel Management Console.
- Clarified the billing distinction: the $25 is a separate recurring Revel charge, not the Kosmos subscription. Kosmos's disclosed price remains $49 month-to-month or $39/month billed annually after its advertised 14-day trial. A Gmail search found no Kosmos receipt or payment confirmation, so the client's current Kosmos trial/conversion/payment status remains unverified.
- Rebekah told Kosmos that she would be away after 10:30 a.m. Eastern on September 9 and asked Kosmos to coordinate routine items with Mark. This does not establish that Mark can sign the Revel order form, approve a recurring fee, or authorize production credentials on Rebekah's behalf.
- No charge, order form, subscription, Revel production-access authorization, API credential, Kosmos connection, Action, schedule, synchronization, product import, Revel write, WooCommerce writeback, payment, shipping, live-site, or staging-to-live change was approved or performed.
- Updated the Phase Two master checklist, ecommerce readiness plan, staging QA record, Kosmos/Revel support record, Policy Review Packet v1.2, and one-product acceptance test so the current gate, costs, NAC candidate, and closed shipping question agree across the active project files.
- Current gate: review Revel's order form, exact location count, cancellation terms, credential scope, and the actual one-way/no-writeback behavior before Todd and Rebekah decide whether to approve Partner Connect. If approved, connect only the Clarkston establishment and run the documented NAC read/import test with schedules and all WooCommerce-to-Revel writeback disabled. Order/customer/inventory writeback remains a later, separately discussed and approved staging test.
- Client-facing follow-up needed: answer Rebekah's fee question in plain English and ask her not to sign or approve the Revel order form yet. Confirm her actual Kosmos plan/billing status from the client-owned account before stating that she has already paid. No email was drafted or sent during this reconciliation.

## 2026-09-09 — Kosmos trial/charge verification attempted

- Rebekah reported on August 25 that the Kosmos account had been created with a 14-day free trial. On a simple calendar estimate, that trial reached its endpoint around September 8, subject to Kosmos's exact activation timestamp and billing rules.
- Todd's Gmail contains no Kosmos invoice, receipt, payment-success notice, or charge confirmation. This means email evidence does not prove that Kosmos charged the client, especially because billing mail may have gone only to Rebekah's account.
- Began a read-only check of the client-owned Kosmos dashboard to verify current plan, trial status, and billing history. Todd stopped Windows browser control with Escape before the Kosmos page loaded, so no dashboard billing evidence was obtained and no further browser actions were attempted.
- Current status remains **unverified**: the trial is likely expired or at its endpoint, but whether it auto-converted, which plan was selected, whether a payment method exists, and whether a charge posted are unknown. No subscription, payment, plan, or account setting was changed.

## 2026-09-09 — Phone-call request sent to Rebekah

- With Todd's explicit instruction, sent Rebekah a short email asking whether she is available for a quick phone call today at 12:30 p.m. Eastern.
- Subject: `Phone call at 12:30 today?`
- Gmail confirmed the message in Sent. No other recipient was copied, and no website, Revel, Kosmos, subscription, payment, or project configuration was changed.

## 2026-09-09 — Kosmos account checked in the in-app browser

- Todd clarified that the Codex in-app browser may be used without separate approval; Chrome and Todd's existing external browser windows remain off-limits unless Todd explicitly authorizes the specific browser action.
- Opened only the Kosmos eSync account in the Codex in-app browser and confirmed it is logged in. The dashboard is accessible and still contains zero Actions.
- The Kosmos `My Account` page shows an account number and a generic `Click here to upgrade your plan or purchase priority support` link. It does not display an active paid-plan name, subscription status, renewal date, invoice, payment method, or completed charge.
- The linked Kosmos billing site is a separate portal and requires a separate login. Its public cart still advertises plans starting at $49/month and a 14-day free trial, but no client-specific invoice or payment history was available without that billing login.
- Assessment: the eSync account itself does not show evidence that the trial converted to a paid plan, but this is not proof that no card charge posted. Todd's Gmail also contains no Kosmos receipt. Actual charge status remains unverified until Rebekah checks the billing login or card statement.
- No upgrade, purchase, subscription, billing-login submission, Action, schedule, synchronization, credential change, or other account setting was made.
## 2026-09-09 — Spreadsheet alternative to inventory integration discussed

- Todd asked whether periodic spreadsheet uploads could replace Kosmos and Revel API access to reduce recurring costs. Reviewed the current 25-product, Clarkston-only scope and official WooCommerce CSV and Clover gateway documentation.
- A manual CSV catalog/inventory workflow is feasible; recommendation is to consider it for the pilot only with a named staff owner and physically reserved online stock, or frequent reconciled stock updates. Monthly shared-shelf inventory snapshots risk overselling. Imports must not restore quantities already sold online but not reflected in Revel.
- Potential avoided integration fees are the documented $49/month Kosmos month-to-month plan plus $25/month Revel Partner Connect for one location, subject to actual billing/cancellation terms. Clover is planned for payments; Todd's quoted $30/month requires merchant-plan verification and is not assumed removable with inventory sync. Existing in-store Revel service remains separate.
- Without a connector, product/price/stock updates and online-order reconciliation into Revel become manual; storefront, checkout, accounts, and separately configured payments/shipping remain viable. Automated Revel writeback has not been enabled or proven, so this is a tradeoff against planned functionality.
- Existing pilot export lacks stock quantities, shipping weights, descriptions, images, and SKU values; a CSV path still requires product enrichment and stable identifier mapping. No architecture switch or account cancellation was authorized. Next decision is whether Mark can own the manual stock/order process; confirm payment fees separately before committing. No client message or system change was made.

## 2026-09-09 — Manual product/order workflow clarification

- Todd requested a step-by-step explanation of spreadsheet products, optional website stock tracking, Clover payment, shipment, and Revel reconciliation. Explained that numeric stock is optional in WooCommerce, but an In Stock flag alone cannot detect sellouts; tracked online quantities can remain hidden from customers.
- Proposed manual workflow: import enriched catalog, WooCommerce records order and tracked stock reduction, configured Clover gateway processes payment, staff fulfills from WooCommerce, and authorized client staff records the inventory departure and appropriate externally paid sale/reporting in Revel using a procedure confirmed with Mark/Revel. No automatic Revel connection exists in this alternative; avoid charging again or deducting stock twice.
- Explained shared-shelf availability, separate payment versus inventory records, manual price/restock updates, returns/refunds reconciliation, and missing export fields. Specific Revel screens/tender procedure remain unverified. No configuration or architecture decision made and no Revel write authorized or performed.

## 2026-09-09 — Shipping charges versus prepaid labels clarified

- Todd asked whether the shipping plugin collects shipping and prepares a label so staff can mail the package. Clarified the two transactions: customer pays the store at checkout; store purchases carrier postage through a configured label service.
- Selected Octolize USPS Live Rates documents checkout rate calculation; label purchasing/printing remains a separate setup and verification item. WooCommerce Shipping is a possible free extension for purchasing/printing USPS labels, with postage charged separately; no installation or service selection was made.
- Intended staff workflow is pack, confirm actual package weight/dimensions and service, purchase/print/attach label, then hand parcel to USPS. Checkout charge and final postage can differ, so representative package/rate testing remains required. No account, payment, shipping, or website setting changed.

## 2026-09-09 — Staff label access and postage billing explained

- Clarified proposed WooCommerce Shipping workflow: order email alerts staff; staff logs into the website order administration to confirm package details and buy/print a prepaid label. Automatic label emailing at order placement is not part of this proposed setup.
- Verified official WooCommerce Shipping documentation: postage purchases charge a saved card on the connected WordPress.com account. Proposed setup uses the client-owned account and business card, origin/package/printer settings, and authorized staff access. Customer shipping money collected through Clover is separate from the business-card postage charge.
- Label extension setup and testing remain pending; no plugin, account, saved payment method, charge, or live setting changed.

## 2026-09-09 — Revel API timing email sent to Mark

- After Todd reviewed the exact draft and explicitly authorized sending it, sent the email to Mark Cobleigh at `lapeerpurchaser@rebekahspureliving.com`, with Rebekah copied at `rebekahspureliving@gmail.com`.
- The draft explains that Blue Nova is traveling for the remainder of this week and plans to work on the Revel/Kosmos connection and controlled staging test Monday, September 14 through Wednesday, September 16.
- Requested that the Revel API key and secret be released to Kosmos by Monday. Mark may complete the Partner Connect request now or early next week, whichever is easier.
- The draft states the expected Revel Partner Connect fee is $25/month for the location and instructs Mark to stop and check with Blue Nova if the form shows any different fee, additional charge, or different terms.
- The draft names `REBEKAH'S NAC 1000mg` as the first controlled product test and explains that product, pricing, inventory, and other mappings will be reviewed before proceeding to the remaining products.
- Gmail confirmed the message in Sent. No fee was approved, and no Revel, Kosmos, WooCommerce, payment, shipping, or staging setting was changed.

## 2026-09-09 — Shop Fullscript promoted to the live main navigation

- At Todd's request, moved the existing Shop Full Script submenu link out of In-Store Products and into the shared live main navigation, immediately after In-Store Products. Standardized the navigation label to Shop Fullscript and preserved its /shop-fullscript/ destination.
- Applied only the targeted live header change. Saved before/after PHP snapshots in wordpress/live-changes/2026-09-09-fullscript-menu and applied the same targeted change to the local theme header so future deployment retains it; no staging deployment was performed.
- WordPress confirmed the file edit. Flushed object cache and purged Breeze static cache and Cloudways/Varnish; confirmed the exact canonical homepage returned HTTP 200 with the new top-level link in an independent anonymous response.
- Visually verified the desktop header and expanded mobile menu in the in-app browser, tested the link to the Fullscript landing page, and checked desktop/mobile widths for horizontal overflow (none). The existing collapsed-menu behavior remains at narrower desktop/tablet widths.
- No client follow-up is required for this change. Earlier reviewed mockup versions were preserved; this was a direct authorized live navigation adjustment.

## 2026-09-09 — Fullscript menu client email screenshot

- Prepared an email draft in chat notifying Rebekah that Shop Fullscript is in the main menu. Added a desktop homepage screenshot with the complete navigation visible, saved to client-handoff/shop-fullscript-desktop-menu-2026-09-09.png. No email was sent.


## 2026-09-09 — Fullscript menu update emailed to Rebekah

- With Todd's explicit send approval, sent the reviewed email titled 'Shop Fullscript added to the main menu' to Rebekah at her established Gmail address, including the desktop screenshot inline. Gmail confirmed the message in Sent (1a08758da74f0a6e). No follow-up is required.

## 2026-09-14 — GSC 1K-click achievement reviewed

- Reviewed Google Search Console's September 12 achievement email for the exact URL-prefix property `https://rebekahspureliving.com/`: Google reports that the property reached 1,000 Search clicks in the preceding 28 days on September 10, 2026.
- Confirmed Rebekah's Health & Nutrition remains an exact Active registry match. The current cloud Search Console report, generated September 8 for the exact domain, independently shows 1,104 clicks from August 9 through September 5, which is consistent with the achievement notice.
- This is a positive informational milestone, not an indexing, security, ownership, or manual-action alert. Recommendation: **no action**; continue routine monitoring. The separately documented `/shop/` noindex remains intentional while the ecommerce work is not live and is unrelated to this notice.
- Sent Todd the required private Slack self-alert summarizing the milestone and no-action conclusion. No website or Search Console setting was changed.

## 2026-09-14 — Mark/Kosmos/Revel connection handoff reconciled

- Reviewed all relevant Gmail messages received since the September 9 API-timing email, including read and unread mail from Mark, Rebekah, Revel, and Kosmos. No new direct Revel or integration-ticket response arrived after September 9; the actionable new message is Mark's September 11 handoff email.
- Revel's case #03058403 message states that the Partner Connect order would be processed within one to two days and that billing will not begin until October 1, 2026. Mark then reported that Revel released the API information directly to Kosmos.
- Mark said Rebekah asked him to take over the WooCommerce/Revel connection. He intentionally stopped before starting anything and requested direction. His screenshot shows a Revel connection named `Rebekahs Health and Nutrition` and the existing Cloudways staging WooCommerce connection selected in Kosmos's Create Action builder; no completed or running Action is evidenced.
- Mark also tried to add Todd through a WooCommerce collaborator screen. No additional WooCommerce.com collaborator is needed for the controlled staging proof because Blue Nova already has Cloudways/WordPress staging access and Kosmos already has its dedicated staging WooCommerce REST connection.
- Kosmos ticket #461373 is the already-resolved question about a second eSync administrator login. Kosmos's September 12 inactivity notice can close without reply; it is unrelated to the current product-sync connection.
- Current gate: verify that the connected Revel account is scoped only to Clarkston, determine whether the product Action can be limited to `REBEKAH'S NAC 1000mg`, verify all schedules and WooCommerce-to-Revel writeback remain absent, and obtain Todd's exact action-time approval before creating the first Action.
- Public read-only checks returned HTTP 200 for the Phase Two staging homepage and Shop. The anonymous WordPress REST root timed out and should be rechecked during preflight before relying on connector calls. No service restart, hosting change, email reply, Kosmos Action, sync, product import, Revel edit, payment, shipping, live-site change, or staging-to-live action was performed.
- Followed the project authentication policy and used the approved `Cody WP Logins` register for one read-only Kosmos login attempt. The stored credential was rejected. Stopped immediately, cleared both fields, and did not retry or change the password. Todd/Rebekah must sign into Kosmos in the Codex in-app browser again or securely refresh the saved credential before Blue Nova can verify the live dashboard.
- Rechecked the Kosmos URL at Todd's request; the in-app browser has no active Kosmos session and redirects to the login page. Did not retry the rejected credential. Clarified that Revel's October 1 date is only the start of Partner Connect billing: Revel processed the order and released access beforehand, so the connection can be used now.

## 2026-09-14 — Kosmos replacement-password request drafted

- Created an unsent Gmail draft to Mark, with Rebekah copied, identifying `rebekahspureliving@gmail.com` as the required client-owned Kosmos eSync dashboard login.
- The draft explains that the previously supplied password no longer works and asks them to provide or reset the current password. It expressly tells them not to email the password and to provide it by phone or another secure method.
- The draft asks that the Kosmos login email remain under Rebekah's ownership and restates the next safeguards: confirm Clarkston-only scope, keep automatic schedules and every WooCommerce-to-Revel writeback disabled, and test only `REBEKAH'S NAC 1000mg` first.
- Draft only; no email was sent, no password was changed, and no Revel, Kosmos, WooCommerce, payment, shipping, or staging setting was changed.

## 2026-09-14 — Kosmos login-error screenshot added to draft

- At Todd's request, added an inline screenshot of the Kosmos eSync login page to the existing unsent Gmail draft to Mark, with Rebekah copied. The screenshot visibly documents the `Incorrect email address or password` error Blue Nova is receiving.
- The password remains masked in the screenshot. The draft was saved and left unsent for Todd's review; no login retry, password change, email send, Kosmos Action, Revel write, sync, or website change was performed.
- Initial insertion was stripped when Gmail saved the message because the draft was in Plain text mode. After Todd reported the missing image, disabled Plain text mode for this draft, reinserted the screenshot, waited for Gmail to save it, and performed a full reload. The reloaded server-saved draft visibly retained `clipboard.png`, and Gmail marks the draft as having an attachment.

## 2026-09-14 — Current ecommerce blocker re-audited

- Rechecked the master checklist, ecommerce readiness record, staging QA record, local readiness index, clean-start plan, and staging scaffold after Todd challenged an inaccurate summary of remaining work.
- Confirmed the same-server staging clone and restore point are complete; the measured 2 GB/1-vCPU retention decision is intentional; all 61 legacy products are quarantined as Draft with zero published products; all seven approved storefront systems, cart, checkout, account structures, operational settings foundation, shipping region, and PO-box validation are implemented on staging.
- Corrected outdated status language in the local readiness index and ecommerce readiness record. Revel credentials have been released to Kosmos and both connections are visible, but no Kosmos Action has been created. The immediate productive-work gate is restored Kosmos dashboard access, followed by Clarkston-only/no-writeback verification and the separately authorized NAC one-product proof.
- Downstream USPS live-rate testing, real-product checkout/order/email/inventory QA, remaining 24-product reconciliation, Fiserv/Clover payment testing, final SEO/analytics, client acceptance, and launch remain open, but they are not unfinished staging-foundation work.

## 2026-09-14 — Rebekah and Mark follow-up reconciled

- Reviewed the newest relevant Gmail conversations from Rebekah and Mark against the Phase Two master checklist. Rebekah replied to the Kosmos access request with a replacement dashboard credential. The credential value was not repeated, saved locally, or copied into project records, and it has not yet been entered or tested.
- No newer direct integration instruction from Mark was found after his September 11 handoff. His existing screenshot still proves only that the Revel and Cloudways staging WooCommerce connections are selectable in Kosmos; it does not prove Clarkston-only scope, a one-product filter, no-writeback behavior, or a completed Action.
- Reviewed the current Fiserv thread. Rebekah received an Authorize.Net DocuSign packet but has not signed it. She asked for written clarification of the true ecommerce processing rate, pass-through and recurring fees, PCI charges, early-termination fee, and premium-card pricing before accepting a three-year agreement. Payment onboarding therefore remains pending and does not block the controlled Revel-to-staging one-product proof.
- For the one-product proof, the remaining access gate is a successful Kosmos login followed by read-only verification of Clarkston-only scope, zero schedules, zero existing Actions, and no WooCommerce-to-Revel writeback. Creating or running the NAC Action still requires Todd's exact action-time authorization under the Revel production safeguard.
- No email was drafted or sent, no credential was stored, and no Kosmos login, Action, schedule, synchronization, Revel write, payment agreement, charge, shipping change, or website change was performed during this reconciliation.

## 2026-09-14 — Replacement Kosmos login verified; Action scope remains unproven

- After Todd gave specific action-time approval, entered Rebekah's emailed replacement credential only into the client-owned Kosmos eSync dashboard. The login succeeded. The password was not displayed, saved locally, or copied into project records; the Gmail tab was returned to Inbox and temporary in-memory credential values were cleared.
- The live dashboard still reports zero Actions. No task or synchronization schedule exists, and no data is currently moving between Revel and WooCommerce.
- Confirmed exactly one selectable Revel connection named `Rebekahs Health and Nutrition` and the intended `Rebekah's WooCommerce — Cloudways Staging` connection. The connection label does not identify an establishment, so it does not prove Clarkston-only scope.
- Followed the unsaved `Revel send standard items to Woocommerce` setup only far enough to inspect its controls. Before Save, the flow exposes only the Action name; it shows no location selector and no single-product filter. Navigated away without saving, then returned to the dashboard and reconfirmed zero Actions.
- Kosmos's official Revel/WooCommerce documentation describes a single-Action test but not a single-product selector. It says the standard-item Action processes qualifying Revel products based on timestamp, online/third-party flag, active state, SKU/barcode, inventory, price, and category. Therefore an NAC-only import cannot yet be guaranteed merely by choosing the standard-item Action.
- My Account still shows only the account profile, account number, and a generic upgrade link. It does not show a plan name, invoice, renewal date, payment method, or completed charge, so Kosmos trial/conversion billing remains unverified.
- Remaining immediate gate: obtain written Kosmos/Revel confirmation that the existing connection is Clarkston-only and determine a safe NAC-only isolation method. Do not save or run an Action until Todd reviews the exact plan and gives separate action-time authorization. No Action, task, schedule, synchronization, Revel write, WooCommerce writeback, subscription change, charge, or website change was made.

## 2026-09-14 — Revel location and NAC-only scope verified read-only

- Used the separate Blue Nova Revel Management Console account from the private `Cody WP Logins` register without displaying or storing its credential. The live console header explicitly lists only **Rebekah's - Clarkston**. This confirms the human admin user is Clarkston-only, but it does not prove the establishment scope of the separate production API credential Revel issued directly to Kosmos.
- Opened `REBEKAH'S NAC 1000mg` read-only in Revel. Verified product ID `46236`, barcode `733739401854`, blank SKU, current price `$27.00`, Active status, and that `Display on online and 3rd party applications` is enabled. No field was changed or saved.
- Rechecked the existing client-supplied Revel export `Product_Export_Establishment_3 (70)_results.xlsx`. All 25 pilot rows are Active and all 25 have `Display on online and 3rd party applications = Yes`. Therefore that flag identifies the entire pilot and cannot by itself guarantee an NAC-only first import.
- The Kosmos standard-item Action showed no product selector before Save, and Kosmos's documentation describes timestamp plus eligibility rules rather than a single-product selector. The first Action remains blocked until the API connection is proven Clarkston-only and an NAC-only filter/timestamp method is proven, or Todd reviews and explicitly approves a revised controlled scope.
- Attempted to prepare a Revel Advanced Product export containing only the online-display flag and product ID. Revel displayed a warning that export would irreversibly assign barcodes/SKUs to products missing them. Canceled the export immediately. No export ran; no Revel SKU, barcode, product, setting, Action, schedule, synchronization, or WooCommerce data changed.

## 2026-09-14 — Kosmos password register updated

- At Todd's explicit request, replaced the rejected older Kosmos eSync password in the private `Cody WP Logins` Google Sheet, `WP Logins!E15`, with the replacement Rebekah sent by email.
- Re-read the saved cell and verified that it exactly matches the replacement credential. The password was not displayed in chat, copied into project files or client notes, or left on the browser clipboard. No other credential row or account setting was changed.

## 2026-09-14 — Fiserv fee response reopened for review

- Andrew Droz replied to Rebekah's written fee questions after the earlier audit checkpoint. His response describes ecommerce pricing, monthly PCI administration, conditional PCI non-compliance charges, waived early-termination and gateway/annual fees, plus chargeback and retrieval fees.
- The new reply materially changes the prior monitoring item from complete to actionable. Added a high-priority, due-today reminder to Todd's private Master Tasks list to review the written terms with Rebekah before any agreement is signed.
- The payment agreement remains separate from the controlled Revel-to-staging product proof. No email was sent or drafted, no agreement was signed, and no Fiserv, Kosmos, Revel, WooCommerce, or mailbox setting was changed.

## 2026-09-14 — Fiserv fee review completed

- Todd confirmed that the review of Fiserv's written fee answers with Rebekah is complete.
- Marked the corresponding item complete in Todd's private Master Tasks list and reconciled the email follow-up workflow state.
- No email was sent or drafted, no agreement was signed, and no payment, Kosmos, Revel, WooCommerce, or mailbox setting was changed as part of this status update.

## 2026-09-14 — First Clarkston Revel-to-staging Kosmos import completed

- Todd explicitly approved proceeding with a reversible protected-staging import even if Kosmos could not isolate NAC to one record. Created the one-way Action `Clarkston staging - Revel products to WooCommerce - manual pilot` and the Task `Clarkston staging manual product import` in the client-owned Kosmos account.
- The saved Action exposed and selected Revel establishment `3 Rebekah's - Clarkston`, used records updated after `2026-09-01T12:05:00`, mapped Revel barcode to WooCommerce SKU, and used the POS category option. No WooCommerce-to-Revel Action was created and no Revel record or setting was changed.
- Verified the Task has no automatic schedule selected. The manual run started at 12:11:28 EDT and completed at 12:43:37 EDT with final status `Done`.
- The final Kosmos transaction log shows 245 qualifying Clarkston records, 245 successful WooCommerce category lookups (`GET 200`), and 245 successful WooCommerce product creations (`POST 201`), with no error-like entries. The cutoff was much broader than the supplied 25-product pilot packet.
- Public WooCommerce Store API verification reports exactly 245 published products after the run. Sample imported records retain the Revel barcode as WooCommerce SKU and price, but show no image, description, weight, or dimensions and are out of stock. The custom Shop template did not list them because its approved website taxonomy/merchandising filters are not yet populated; the individual products and Store API are nevertheless publicly reachable on the noindex Cloudways staging hostname.
- Exact Store API lookup for NAC barcode `733739401854` returned no product, so the September 1 cutoff excluded the selected `REBEKAH'S NAC 1000mg` proof item. The run proves one-way Clarkston transport and usable logs, but it does not pass the NAC-specific field and inventory acceptance test.
- Followed the project credential rule and tried the exact current staging WordPress row from `Cody WP Logins` once. WordPress rejected the stored username/password, so no retry, password reset, product deletion, or quarantine change was attempted. Automatic schedules remain off and the completed import did not write to Revel.
- Immediate next step: restore Blue Nova's staging WordPress administrator access, change the 245 newly imported products to Draft/quarantine without deleting them, then choose the smallest verified cutoff or other Kosmos method that includes NAC and rerun the NAC-specific proof. Do not run a broader import or enable any WooCommerce-to-Revel writeback first.

## 2026-09-14 — Imported catalog field audit completed

- Audited all 245 published products returned by the staging WooCommerce Store API and compared their barcodes with the 25-row Revel pilot workbook.
- The import is clean at the record and identifier level: 245 unique product IDs, 245 unique barcode-based SKUs, no blank SKUs, no duplicate SKU groups, no duplicate-name groups, no blank names, and no zero-price records. Prices range from $1.50 to $66.00.
- Customer-facing content is largely absent: 243 names remain in POS-style all caps; no images, short descriptions, brands, tags, attributes, variations, weights, or dimensions were transferred. Only one product has any description, and that value is only `1 fl oz`.
- Category transfer was partial. Only the two Beekeeper's products received `Immune Support`; the other 243 products are uncategorized. This indicates exact POS-to-WooCommerce category matching can work but the approved website taxonomy still needs deliberate mapping.
- All 245 products are valid/purchasable WooCommerce records but are out of stock, so the product Action did not populate sellable inventory. Inventory requires a separate controlled one-way test before checkout QA.
- Exact barcode comparison found 0 of the 25 approved pilot products in this 245-product result. NAC remains absent. The September 1 cutoff selected a different recently updated Clarkston population; the connector itself did not fail.
- Corrected the working approach: the 245 records do not need to be deleted or treated as a major failure. After WordPress access is restored, quarantine them recoverably and use one imported record to complete mapping/overwrite testing while the exact 25-product inclusion method is resolved.
- Created `PHASE-TWO-KOSMOS-IMPORT-FIELD-AUDIT-2026-09-14.md` as the focused evidence and next-work record. No WordPress product, Kosmos Action, schedule, Revel record, payment, shipping, or live website setting was changed during this read-only audit.

## 2026-09-14 — Staging access and source-content clarification

- Corrected ownership: Blue Nova controls and is responsible for Cloudways staging and staging WordPress access. Mark and Rebekah do not need to supply or repair that access.
- Verified the existing Blue Nova `Cody` administrator session reaches the staging WordPress dashboard and product editor. The earlier rejected stored-credential attempt remains a credential-register maintenance issue, not a client dependency.
- Opened the 245-product staging catalog and inspected `HRBPHRM NERVOUS SYSTEM 1OZ` without saving changes. WooCommerce shows the raw POS title, barcode `090900000026` as SKU, `$19.00` regular price, simple-product type, out-of-stock status, `Uncategorized`, no brand, no image, no short description, and only `1 fl oz` in the long description.
- Opened Revel product ID `18872` read-only and confirmed the source has the same raw title, barcode, `$19.00` price, and `1 fl oz` description. Revel places it in `AdditionalItems > AdditionalItems` with product class `Herb Pharm`; its Revel SKU and manufacturer are blank. Active and Track in inventory are enabled. No Revel field was edited or saved.
- The client-supplied 25-row pilot spreadsheet has blank description and SKU fields for all 25 products. The lack of customer-facing copy is therefore not evidence that Kosmos dropped completed spreadsheet descriptions; those descriptions were never present in the supplied export. The imported population is raw Clarkston POS data and requires WooCommerce-side merchandising/enrichment.
- The representative mapping is now proven: name, Revel barcode-to-WooCommerce SKU, description, and selling price transferred. Website category/brand/image/short description did not. Stock requires its own one-way Revel-to-WooCommerce inventory action; it was not part of the product-creation run.
- The 245 imported barcodes still overlap 0 of the 25 approved pilot barcodes, so every product in this first run is outside the intended 25-product pilot population. They remain useful mapping evidence and can be quarantined recoverably; no product, Revel record, Kosmos Action, schedule, payment, shipping, or live-site setting was changed during this inspection.
- Next action requiring Todd's approval: recoverably quarantine the 245 published imports and use one representative imported record for controlled overwrite/mapping tests while the exact NAC inclusion method is resolved.

## 2026-09-14 — Import terminology clarified

- Replaced the shorthand “September 1 cutoff” with its exact meaning: Kosmos requested Clarkston Revel products updated after September 1, 2026 at 12:05 PM. The Task did not import from the 25-row spreadsheet or use that spreadsheet as an inclusion list, so Revel returned 245 different records and none of the approved 25.
- Confirmed the description counts in plain language: all 25 spreadsheet description cells are blank; 244 of the 245 WooCommerce imports have no long description; one has only `1 fl oz`; and all 245 have no short description. No customer-ready marketing description has been found.
- Clarified classification for the representative record: `HRBPHRM` is part of the raw product name, not the manufacturer field. Revel places it under `AdditionalItems > AdditionalItems` with Product Class `Herb Pharm`; manufacturer and Revel SKU are blank. WooCommerce places it in `Uncategorized` with no brand.
- Clarified inventory language: Revel's enabled `Track in inventory` control is a setting, not a quantity. WooCommerce shows quantity `0` / Out of stock. The completed product-creation Task did not transfer or rename an on-hand quantity; inventory still requires its own controlled one-way test.

## 2026-09-14 — Revel on-hand inventory and brand-source fields verified

- Opened Revel's Clarkston Product Inventory report read-only and searched the representative barcode `090900000026`. Revel reports `3.0000` units of `HRBPHRM NERVOUS SYSTEM 1OZ` on hand, while the corresponding staging WooCommerce product remains quantity `0` / Out of stock. This proves the on-hand value exists in Revel and was omitted because the completed Kosmos product-creation Action did not include the separate inventory synchronization.
- The same Revel inventory record separates `Class` (`Herb Pharm`) from `Primary Vendors` (`Herb Pharm PETE`). Primary Vendor is purchasing/supplier data and must not be assumed to be the public WooCommerce brand. For this item, Product Class is the cleaner brand candidate; manufacturer is blank. Audit the wider pilot population before adopting Product Class as a global brand rule, and normalize values before publishing.
- Planned next controlled integration step is a manual, Clarkston-only, Revel-to-staging-WooCommerce inventory update matched to the already imported barcode/SKU, with no automatic schedule and no WooCommerce-to-Revel writeback. First compare the exact Revel on-hand value with the resulting Woo quantity, then document update/overwrite behavior before expanding scope.
- The current saved Kosmos login was rejected once during the live verification attempt. Stopped immediately without retrying or changing the password. No Kosmos Action was created or run, and no Revel or WooCommerce record was changed.

## 2026-09-14 — Revel product-export and inventory triggers researched

- Reconciled the completed import, live Revel evidence, prior pilot records, and Kosmos's current official Revel-to-WooCommerce documentation. `Display on online and 3rd party applications` is the product-level eligibility gate; inventory on hand by itself does not put an item online, and a zero quantity does not exclude an otherwise qualifying item.
- Product creation/update additionally requires Active status, a Revel SKU or barcode, name, category, nonnegative price, nonnegative inventory value, and a qualifying `product_time_stamp` within the Action date window. Description and weight are optional. The Kosmos Task must then run manually or on a schedule; this project's Tasks remain manual.
- Inventory is separate. A Standard/Matrix Inventory Action processes eligible existing products with a new `inventory_time_stamp`, using Revel SKU/barcode to match WooCommerce SKU. Revel inventory changes from in-store sales, manual adjustments, received purchase orders, or supported online orders update that timestamp.
- Confirmed that Revel custom Online Menus do not filter Kosmos product exports; Kosmos says they affect which items may be included in Revel web orders. Product Class, Primary Vendor, and stock quantity alone are also not export gates.
- Kosmos does not document that clearing the online/third-party setting automatically unpublishes or deletes an existing WooCommerce product. Add a specific disable/remove acceptance test and use an explicit WooCommerce hide/draft procedure until that behavior is proven.
- Updated `PHASE-TWO-KOSMOS-IMPORT-FIELD-AUDIT-2026-09-14.md` with the verified selection, product timestamp, inventory timestamp, execution, and removal rules. No Revel, Kosmos, or WooCommerce setting or record was changed.

## 2026-09-14 — Pilot workbook matched against live Revel

- Compared every one of the 25 workbook barcodes with Clarkston's live Revel Product Inventory in read-only mode. All 25 resolve to matching Revel products; 21 names match exactly and four differ only by doubled spaces in the workbook. All 25 match visible product class, primary vendor, inventory unit, and selling price.
- Opened `REBEKAH'S NAC 1000mg` (product ID `46236`) read-only and verified that its live category/subcategory, blank description, barcode, blank SKU, product cost, price, primary vendor, vendor item ID, and reorder values match the workbook. This establishes that the workbook is grounded in the actual Revel product records, while `Status` and `Remarks` are working-sheet tracking columns.
- Clarified that Product Inventory's `Cost / Inv Unit` is not the same field as the product-detail `Cost`/default reorder price. Seven products have different inventory-unit costs; this does not show a bad spreadsheet value. No Revel, Kosmos, WooCommerce, or workbook value was changed.

## 2026-09-14 — Complete Revel/Kosmos/WooCommerce data map created

- Inspected all 25 Clarkston Revel product-detail records and the matching live Product Inventory rows in read-only mode. Confirmed their Revel IDs, barcodes, current Revel SKUs, names, product/subcategory paths, price, on-hand quantity, description, product class, cost, tax state, UOM, inventory tracking, purchasing fields, manufacturer, and related control states without saving any Revel change.
- Created `PHASE-TWO-REVEL-KOSMOS-WOOCOMMERCE-DATA-MAP-2026-09-14.md` as the canonical field-ownership, connector-direction, transformation, overwrite, missing-data, and barcode-by-barcode pilot map. The map separates Revel operational data, Kosmos Action behavior, and WooCommerce-owned presentation fields.
- Adopted barcode → Woo SKU as the pilot's stable identifier. Current live Revel SKU is blank for 23 products, equals the barcode for Energy, and is `4005401` for Ear Clear. Vendor Item ID remains purchasing data and must not replace the barcode-based website identifier.
- Recorded the September 14 on-hand snapshot: 24 of the 25 products have positive stock, Energy has zero, and the total is 116 units. These are reconciliation evidence for the first Standard Inventory Action, not fixed website content.
- Defined approved-term category translations and product-specific proposals. Spirulina remains intentionally unresolved because the approved taxonomy has no exact equivalent for `Greens & Superfoods`; do not manufacture a health claim to force it into a category. Rebekah must approve the consolidated category exceptions before publication.
- Confirmed the correct next proof: use one imported staging product to establish Product Action overwrite behavior without changing Revel, then run one manual Clarkston-only Standard Inventory Action and reconcile the matching barcode, quantity, stock status, and logs. Automatic schedules and every WooCommerce-to-Revel Action remain disabled.

## 2026-09-14 — Pre-action Phase Two status reconciled

- Reconciled the master checklist, staging QA, Kosmos import audit, 25-product live Revel audit, field map, payment correspondence, and shipping decisions into one current checkpoint before additional integration actions.
- Confirmed the completed foundation: protected same-server staging and restore point; all seven approved ecommerce systems implemented; WooCommerce active; 61 legacy products retained as Draft; Blue Nova staging administrator session working; dedicated WooCommerce/Kosmos REST connection created; Revel establishment `3 Rebekah's - Clarkston` proven in the Action and logs; and one manual one-way Product Action completed with schedules off and no WooCommerce-to-Revel Actions.
- Confirmed the first Product Action created 245 published but out-of-stock staging products selected by the September 1 product timestamp, with 245 unique barcode-based SKUs and prices and no transport errors. None overlaps the approved 25-product set. These records remain protected by the noindex staging environment but still need recoverable Draft/quarantine handling before broader testing.
- Confirmed the 25-product pilot workbook matches all 25 live Clarkston Revel records by barcode and price. Current read-only inventory snapshot is 116 units total, with 24 positive quantities and Energy at zero. All 25 descriptions are blank; images, verified shipping weights/dimensions, ingredients, label content, directions, and warnings are not supplied. Website category proposals exist, with Spirulina and several product-specific exceptions requiring consolidated client approval before publication.
- Current technical uncertainties are limited and explicit: latest Kosmos dashboard credential/session must be reverified before another Action; Kosmos trial-to-paid billing/charge status is not proven; Product Action overwrite behavior is not tested; Standard Inventory Action has not run; exact selection method for the 25 Product records is unresolved; and clearing Revel's online/third-party flag is not proven to hide an existing Woo product.
- Shipping decisions are established as USPS Ground Advantage and Priority Mail, Clarkston origin, two-business-day handling, continental U.S. only, and no PO boxes. The USPS business account exists, but Developer Portal/API app credentials, final live-rate plugin setup, accurate packaged weights, and rate/packing tests remain incomplete.
- Payment remains a later launch gate: no active staging gateway, no signed/confirmed final Fiserv/Clover or Authorize.Net integration product in the project record, and no secure merchant integration credentials. No real payment, order, refund, postage, production customer, or Revel write is authorized or complete.
- Agreed next sequence for Todd's review: verify Kosmos access and billing screen without accepting charges; recoverably quarantine the 245 staging imports; run one Woo-only overwrite-preservation test; inspect and approve the exact one-way Standard Inventory Action scope, then reconcile at least one barcode against live Clarkston quantity; resolve the safest exact-25 inclusion method; create/synchronize the 25 as Draft; apply the canonical category/brand map; research available authorized content and send Rebekah one consolidated gap/approval list; then configure shipping and later payment/order tests under their separate gates.

## 2026-09-14 — Todd clarified billing, content, and imported-product responsibility

- Todd confirmed Kosmos subscription billing is Rebekah's account responsibility, not a Blue Nova deliverable or project blocker. Blue Nova will not approve a plan, charge, or subscription change; it only needs functional dashboard access for authorized integration work.
- Product ingredients, allergens, Supplement Facts, directions, warnings, descriptions, imagery, and similar customer-facing enrichment belong in WooCommerce, not Revel. Blue Nova may use a controlled import/script after overwrite behavior is proven; Revel remains the operational source for identifiers, price, availability, and inventory and stays read-only by default.
- Clarified that moving the 245 broad-import records to Draft is a reversible staging-containment option, not a technical requirement for the connector to work. Because they are out of stock and staging is noindex/protected, one can remain available for controlled testing. The broad population must be hidden or otherwise excluded before client review or launch so it cannot contaminate the approved 25-product catalog, search, APIs, or QA counts.
- Proposed simplifying category approval: Blue Nova applies the obvious existing-category mappings based on verified product identity and asks Rebekah only about genuinely ambiguous placement or the creation of a new category, principally Spirulina and Ear Clear. This proposal is not yet recorded as final client approval.

## 2026-09-14 — Ongoing automation model and Standard Inventory proof

### Eligibility-control clarification

- September 14: Todd explicitly authorized sending the product-removal support email. Sent from tbailey@bluenovainc.com to the verified Kosmos support address support@kosmoscentral.freshdesk.com with subject `Revel → WooCommerce: automatic removal when online availability is unchecked`. Gmail confirmed SENT, message/thread `1a0a156fd3e85ce4`. The email asks about unchecking online eligibility, stale stock/purchasability, inactive/deleted products, re-enabling and content preservation, supported automation and any extra cost, and effects on other channels. Awaiting Kosmos's response; no paid work or configuration change authorized. The local draft file now records the sent status and account identification included in the email.

- Todd explicitly identified automatic removal after clearing the online checkbox as a required known behavior. A focused official-documentation check still found no stated WooCommerce unpublish/delete action. Prepared `KOSMOS-PRODUCT-REMOVAL-QUESTION-DRAFT-2026-09-14.md` covering removal, stale inventory/purchasability, inactive/deleted items, re-enabling without duplication or enrichment loss, supported automation/cost, and isolation from other channels. Draft is local and unsent. Resolution requires a specific vendor answer followed by a controlled test; no production Revel flag was changed.

- Todd asked whether the Revel online/third-party checkbox is the switch for website pulls and whether changing it is isolated from other operations. Rechecked Kosmos's official WooCommerce criteria: Active plus Display on online and 3rd party applications are eligibility conditions; the timestamp window and manual/scheduled Kosmos Task determine when qualifying records are processed.
- The checkbox is not proven exclusive to this WooCommerce connection. Other online/third-party integrations may consume it; their actual use must be checked before changing it. Clearing it is also not proven to unpublish an existing WooCommerce item and may stop subsequent stock updates for that item. To pause this connector, control its Kosmos Tasks/schedules instead of changing production Revel product flags. No Revel checkbox or connector setting was changed during this clarification.

- Todd clarified that Phase Two must automate recurring catalog additions, inventory changes, and product retirements; it cannot depend on manually rebuilding every new item in WooCommerce. Updated the canonical Revel/Kosmos/WooCommerce data map with the required product schedule, inventory schedule, Woo-side transformation/protection rules, exception queue, and retirement reconciliation model. Missing regulated label facts and photography still require one authoritative source before automation can reuse them.
- Restored functional access to the client-owned Kosmos dashboard using the current securely stored credential. No credential was displayed or saved locally.
- Created the one-way Action `Clarkston staging - Revel inventory to WooCommerce - manual pilot` and the unscheduled Task `Clarkston staging manual inventory sync`. The Action is restricted to establishment `3 Rebekah's - Clarkston`, uses inventory records updated after `2026-09-01T00:00:00`, and matches Revel barcode to WooCommerce SKU. No WooCommerce-to-Revel Action or automatic schedule was enabled.
- Ran the manual inventory Task. Kosmos event group `27284`, event ID `086620f6-e41a-4372-9cca-ff74256be1c5`, completed with dashboard status `Done`, 250 selected inventory records, and 513 report entries.
- Final-log reconciliation found 7 successful WooCommerce inventory updates (`PUT 200`), 233 selected Revel inventory records with no existing Woo product match, and 10 consecutive Woo product searches that returned HTTP `503`. Kosmos reports `Done` despite those 10 request failures, so a clean smaller-window rerun is required before scheduling.
- Verified an actually updated record end to end: `HERPHA SMOKER ASSIST 10Z`, barcode/SKU `090900000019`, is `2.0000` units on hand in live Clarkston Revel and quantity `2` / `2 in stock` in staging WooCommerce. This passes exact quantity transfer for a selected barcode-matched standard product.
- Corrected the earlier comparison: `HRBPHRM NERVOUS SYSTEM 1OZ` (`090900000026`) was absent from this Action's 250-record inventory timestamp population and has no update entry in the final log. Its current staging quantity `1` / Out of stock was not produced by this run and cannot be used to judge the Action.
- The approved one-unit website safety buffer is not implemented yet: the verified two-unit product is currently customer-available at quantity two. Kosmos exposes no offset control in this Action, so the buffer requires a separate WooCommerce-side rule and acceptance test.
- Next technical sequence: perform one Woo-only Product Action overwrite-preservation test; repeat inventory using a small current incremental window with zero Woo/API errors; implement and test the one-unit buffer; resolve exact-25 product inclusion; then enable carefully timed one-way schedules only after the pilot passes. Automatic removal still requires a tested online-eligibility disable path or a Woo-side reconciliation job.

## 2026-09-14 — Mark clarification sent; staging overwrite test started

- At Todd's explicit instruction, sent Mark Cobleigh an email asking which other websites, ordering services, marketplaces or third-party apps use Revel's online/third-party checkbox for Clarkston, and whether staff already use that checkbox operationally. Asked him to confirm if none; explicitly requested no Revel changes. Subject: `Quick question about Revel’s online/third-party checkbox`. Sent to lapeerpurchaser@rebekahspureliving.com from Todd; Gmail SENT message/thread 1a0a1609da3cc1f4. Mark's operational answer and Kosmos's previously requested connector/removal answer are both pending.
- Prepared and saved title/long-description/short-description markers on staging Woo product 1479181 (barcode 090900000026), preserving original values for cleanup. See PHASE-TWO-PRODUCT-OVERWRITE-TEST-2026-09-14.md.
- Existing Product Task 27283 processed zero records because its incremental timestamp had advanced. Created isolated manual replay Task 27285 with the same verified one-way Action and original September 1 boundary; confirmed **In progress**, 245 records, at approximately 15:32 EDT. Event 0fc5f75f-367f-4da4-a8ca-6868b24ec5e4.
- No Revel production changes, automatic schedules, writeback actions or paid upgrades enabled. Reauthenticated Kosmos using the securely saved login without exposing or retaining a local credential copy.
- Todd may close/sleep the laptop while the cloud run continues. Next session: check completed log and target fields, document overwrite behavior, restore original test fields, and update acceptance test C2. Test outcome and cleanup remain pending.
- Todd additionally requested the run duration. Created thread follow-up `kosmos-test-duration`, checking every 15 minutes when available, read-only against task 27285. Report actual log start/end and elapsed time (label approximate if needed), record failures, and pause the follow-up after reporting completion. Sleeping may delay notification but must not inflate the calculated runtime.
- Todd requested a future email for Rebekah's approval covering inventory synchronization timing and the one-unit website stock buffer when this stage is ready. Explain the measured update delay, proposed schedule, buffer behavior and overselling limitations, with test evidence. Draft for Todd's review first; no email sent now. Prior internal agreement on the buffer is not evidence of Rebekah's client approval; obtain that approval before live activation.
- At 16:17 EDT, replay 27285 remains running (~45.5 minutes, 657 entries). Refreshed test product proves title and long description have reverted to Revel values while short-description marker remains. Partial log shows existing-item POST 400 responses followed by successful PUT 200 updates; do not misreport those as server outages. Final duration/reconciliation and marker cleanup pending. Immediate technical priorities: protect/enrich Woo presentation fields automatically, diagnose prior inventory 503s, then test incremental inventory and proposed buffer before scheduling. No fixes or additional runs performed in this status review.
- Todd corrected sequencing: defer the one-unit buffer test and Rebekah approval email to later pre-launch work; do not treat them as current staging blockers. Continue current catalog mapping/content protection and connector reliability work first. This supersedes the immediately preceding priority ordering, not the eventual pre-launch acceptance requirement.
- Continued staging work: updated acceptance C2 with successful target PUT and observed title/long-description overwrite; created and regression-checked scripts/Get-KosmosRunSummary.ps1 against both existing product and inventory reports. Correctly distinguishes expected POST 400 existing-item responses from the 10 actual inventory GET 503 failures.
- Prepared local staging-only catalog-presentation plugin candidate using approved source-referenced barcode mappings; no deployment, activation, registry import or runtime validation yet. No PHP CLI was available on PATH or checked common runtime locations. Do not call content protection fixed until runtime and end-to-end tests pass. Bulk content intake remains required; no invented product copy or manual per-sync correction workflow introduced.
- Inspected Wordfence Live Traffic (security-only; no entries) and Woo log inventory (no September 14 fatal-error file). These did not identify the 503 root cause. Recorded current PHP/memory settings in the canonical map; did not change limits, security, hosting or paid services. Cloudways timestamp-matched server logs are the next diagnostic source.
- Replay still In progress at 16:26:24 EDT, with last activity 16:26:17 and 676 report entries (~54 minutes elapsed from initial running observation). No overlapping sync started and no changes made to the active experiment. Final log reconciliation, cleanup, and protection deployment/testing remain pending.
- Todd emphasized shared-server protection for both staging and production. Immediate low-impact public homepage checks returned live HTTP 200 in 0.835 seconds (TTFB 0.703s), staging HTTP 200 in 25.117 seconds (TTFB 25.037s). Production responded normally in this sample; staging is severely slow. These single requests do not prove sustained uptime, uncached production health, or server headroom. Do not begin another sync, deploy during the active replay, disable security, restart shared services or resize by implication. Check shared CPU/RAM/PHP-worker pressure and both applications before subsequent workload. This is production-impact safety guidance, not authorization to promote staging ecommerce changes to live.
- Todd clarified that staging-only slowdown is acceptable and requested an approximately 48-hour completion push with minimal need for his time. Prioritize catalog/content protection and inventory reliability, and batch compatible field tests rather than serial 30–50-minute runs. Updated master checklist with this explicit override and a combined acceptance matrix. Shared-production safeguards, paid-change approval and Revel read-only rules remain unchanged; buffer/approval email remain later pre-launch work.
- Restored expired Kosmos session securely and checked replay again at approximately 16:33 EDT: still In progress, 746 log entries, last activity 16:33:50. No extra run launched. Planned targeted protection-hook tests ahead of any broad replay; final duration not yet available.
- Prepared a combined 13-check isolated PHP policy harness covering copy protection, operational-field isolation, leading-zero barcode matching, source provenance, empty-field rules and production no-op. Harness is NOT executed: no PHP CLI/common portable runtime or WSL available, and official Windows PHP download endpoint was unreachable. Actual WordPress persistence/sanitization and deployment remain pending; do not report the local candidate as tested or fixed. Existing log-summary regression checks remain passed separately.
- Incorporated Todd-requested side-conversation inbox handoff: Mark's Sept 14 15:46 EDT reply says Kosmos appears to be the only Revel connection (Gmail 1a0a175116c312c3). James/Kosmos's 15:53 reply, ticket 461388 (Gmail 1a0a17bb7b706419), says clearing online eligibility stops stock/price/all sync; inactive/deleted items remain online; standard connector does not remove products; rechecking resumes qualifying updates. These supersede earlier pending-answer statements. Custom removal requires a consultation/quote; none approved.
- Todd indicated normal sold-out products may remain visible as Out of stock if non-purchasable. Added zero/replenishment acceptance checks, distinct from deliberate online withdrawal and from the deferred buffer. Keep eligibility enabled for zero/restock updates. No live Revel test changes without specific authorization. Updated canonical map and master checklist; no new email, paid action, production change or test run performed by this handoff.
- Finished replay reconciliation: task 27285 completed September 14 at 16:52:29 EDT; approximate duration 80m27s from first running observation. Final log: 245 distinct successful product updates, 245 expected existing-product responses, zero unexpected HTTP errors. Dashboard 988 entries versus downloaded 984 JSON records. Original overwrite-test title/description restored by connector; cleared surviving short-description marker and verified all three original fields after editor reload. Duration heartbeat paused after reporting completion.
- Began authorized staging-only zero/restock simulation on Woo 1479193 (090900000019). Baseline quantity 2, manage stock yes, backorders no, instock, price 19. Entered zero and clicked Update, then in-app browser control became unavailable across original and fresh tabs. Saved zero could not be verified and restoration remains pending. FIRST next action is inspect/restore quantity 2 and verify front end before further tests. No production/Revel writes performed. Zero/restock test NOT passed; do not confuse browser/tool connectivity failures with a proven live-site outage.
- Todd instructed Codex to handle browser reopening itself. Reopened the exact staging editor in the in-app browser, confirmed zero quantity persisted, entered original quantity 2 and submitted Update. Verification attempts via editor reload and a fresh canonical product tab hit browser-control deadlines; public read-only endpoint check also attempted. Restoration remains submitted/unverified, not completed. No user browser task, new sync, live-site change or Revel change is needed/authorized by this recovery.
- Continuation: confirmed prior restoration in fresh admin state: quantity 2, managed stock on, backorders no. Woo also reported a stale-edit conflict and explicitly said product 1479193 had 2 units. Attempted a new authorized zero-stock simulation; browser target closed during save/observation. Fresh public/editor tabs and alternate screenshot recovery all timed out. Latest quantity is UNKNOWN after that new attempt; first next action is read back/restore 2. Do not repeat zero simulations until reliable readback exists. Updated acceptance C2 from provisional to completed replay evidence (245 successful updates, title/long overwritten, short survived; markers cleaned). No additional sync, deployment, Revel write, production change, or paid action performed. Content-protection candidate remains undeployed and untested.
- Todd clarified to keep moving with grouped tests, not stop or serialize long full-catalog runs. Recovered saved zero on Woo 1479193; refreshed canonical product page rendered Out of stock without Add to cart. Restored 2 and verified after editor reload (managed stock true/backorders no). This supersedes unknown-quantity warning; frontend restock and cart rejection still pending. No new zero simulation or bulk import needed. Static code review fixed whitespace-only provenance acceptance and expanded local copy-protection harness to 22 combined checks (routes, identity, fields, zero/new price). PHP runtime still unavailable, so no runtime pass/deployment claimed. No live/Revel changes or paid actions.
- Todd approved proceeding. Resolved local PHP blocker using official checksum-verified portable PHP 8.2.33 in a temporary folder, matching staging's reported PHP version. Plugin syntax PASS and all 22 isolated policy checks PASS. README updated; prior runtime-unavailable status superseded. Actual WordPress sanitization/persistence, controlled wc/v1 integration, and bulk approved-content intake remain pending; no deployment or registry import performed. Restocked canonical page still could not be inspected due browser-control timeouts; admin quantity 2 remains latest verified persisted baseline. No new sync, Revel write, live-site change, paid service, or system configuration change.
- Following explicit approval, verified fresh canonical product page shows 2 in stock and Add to cart, completing Woo-only zero/restock product-page display check. Installed and activated RHN Catalog Presentation (Staging) v0.1.0; verified active Plugins row/Deactivate control. Exact deployed path and ZIP hash recorded in README. No registry populated or product-copy changes; module remains pass-through without entries. Rollback is plugin deactivation. Added local-only bulk-intake parser/admin form candidate; lint and 12 isolated validation checks PASS (34 total with policy checks). Intake module is not wired/deployed. Actual wc/v1 protected update, registry persistence/rollback, HTML sanitization, and bulk source intake remain incomplete. No broad sync, Revel write, live-site change, paid action, or customer transaction.
- Todd clarified not to re-request approval for already approved staging steps. Continued autonomously: deployed v0.2.0 then v0.2.1 in the same plugin directory, wired bulk intake and a one-shot fixed-product internal REST proof with durable baseline/recovery. All eight actual wc/v1 checks PASS (HTTP200, title, sanitized long copy, short copy, exact barcode, price20, quantity1/backorders, source metadata). Original product fields/metadata restored and read back; fresh canonical page shows original title, $19, quantity2/Add to cart, no markers. Intake UI rejected operational stock field, previewed/saved verified synthetic registry entry, restored previous registry verified; no real product entry created and no test rule remains active. Local 34 checks still PASS. Updated acceptance/map/README. Scope distinction: actual internal Woo REST protection verified, not a new Kosmos transport run or supplier feed. Existing source map says all25 descriptions blank and label/images unavailable; real approved content sourcing remains a prerequisite for bulk enrichment. No live/Revel changes, new bulk run, schedules, paid actions or outbound email.
- Continued catalog/inventory reconciliation: current Woo UI306 total,245 published/61 drafts. NAC exact barcode search733739401854 returns no products across all statuses. None of25 pilot barcodes found in completed replay request URLs; do not generalize this to a full current Woo audit. Securely restored expired Kosmos session using saved sheet row; sheet unchanged. Product Action27772 visibly has Clarkston3, September1 cutoff, barcode matching and POS categories, no exposed exact-product filter. No settings changed/run started. Prepared local unsent support draft asking deterministic25-product backfill, adoption of barcode-keyed Draft bootstrap, initial inventory,250-record cap/pagination and failed-request watermark handling. Updated stale map completion statements. Main unresolved issue is exact pilot inclusion and clean inventory transport, not repeating the completed presentation tests.
