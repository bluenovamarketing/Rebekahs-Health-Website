(async()=>{
  const response=await fetch("second-mockup.html");
  if(!response.ok)throw new Error("Unable to load original second mockup sections");
  ...
  mounts.forEach(mount=>{
    ...
    let section;
    if(name==="pathway")section=clone(".pathway");
    if(name==="shipping")section=clone(".shipping");
    if(name==="events"){
