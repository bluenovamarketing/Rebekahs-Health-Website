(() => {
  ...
  chromeMenu.addEventListener('click', () => {
    const willOpen = chromeMenu.getAttribute('aria-expanded') !== 'true';
    chromeMenu.setAttribute('aria-expanded', String(willOpen));
    chromeNav.classList.toggle('open', willOpen);
    if (!willOpen) closeDropdowns();
  });

  chromeNav.querySelectorAll('a').forEach((link) => {
