<?php
/**
 * Plugin Name: Blue Nova Phase Two Staging Installer v1.5
...
function blue_nova_phase_two_v1_5_install_theme_files( $backup_root ) {
...
function blue_nova_phase_two_v1_5_activate_woocommerce() {
...
function blue_nova_phase_two_v1_5_isolate_legacy_products( $backup_root ) {
	global $wpdb;
...
function blue_nova_phase_two_v1_5_disable_gateways() {
...
function blue_nova_phase_two_v1_5_configure_core( $backup_root ) {
	$options = array(
	...
	blue_nova_phase_two_v1_5_disable_gateways();
...
function blue_nova_phase_two_install_v1_5() {
	...
	if ( get_option( 'blue_nova_phase_two_staging_patch_v1_5' ) ) {
		return;
	}

	$backup_root = trailingslashit( WP_CONTENT_DIR ) . 'uploads/blue-nova-phase-two-backups/v1.5-' . gmdate( 'Ymd-His' ) . '/';
	$files       = blue_nova_phase_two_v1_5_install_theme_files( $backup_root );
	$woo_state   = blue_nova_phase_two_v1_5_activate_woocommerce();
	$products    = blue_nova_phase_two_v1_5_isolate_legacy_products( $backup_root );
	$pages       = blue_nova_phase_two_v1_5_configure_core( $backup_root );

	flush_rewrite_rules();
	update_option(
		'blue_nova_phase_two_staging_patch_v1_5',
		array(
...
register_activation_hook( __FILE__, 'blue_nova_phase_two_install_v1_5' );

/** Leave an auditable, non-sensitive completion notice for administrators. */
function blue_nova_phase_two_v1_5_admin_notice() {
	$record = get_option( 'blue_nova_phase_two_staging_patch_v1_5' );
	if ( ! $record || ! current_user_can( 'manage_options' ) ) {
		return;
	}
	?>
	<div class="notice notice-success is-dismissible"><p><strong>Phase Two staging v1.5 installed.</strong> Final responsive empty-cart cleanup is installed; approved ecommerce templates remain isolated from payments and external connections.</p></div>
	<?php
}
add_action( 'admin_notices', 'blue_nova_phase_two_v1_5_admin_notice' );
