<?php
/**
 * Plugin Name: Blue Nova Phase Two Staging Installer v1.3
 * Description: Installs approved ecommerce Systems 03-07, safely activates WooCommerce core, and isolates legacy products on Rebekah's Cloudways staging site.
 * Version: 1.3.0
...
function blue_nova_phase_two_v1_3_install_theme_files( $backup_root ) {
...
function blue_nova_phase_two_v1_3_activate_woocommerce() {
...
function blue_nova_phase_two_v1_3_isolate_legacy_products( $backup_root ) {
	global $wpdb;
...
function blue_nova_phase_two_v1_3_disable_gateways() {
...
function blue_nova_phase_two_v1_3_configure_core( $backup_root ) {
	$options = array(
	...
	blue_nova_phase_two_v1_3_disable_gateways();
...
function blue_nova_phase_two_install_v1_3() {
	...
	if ( get_option( 'blue_nova_phase_two_staging_patch_v1_3' ) ) {
		return;
	}

	$backup_root = trailingslashit( WP_CONTENT_DIR ) . 'uploads/blue-nova-phase-two-backups/v1.3-' . gmdate( 'Ymd-His' ) . '/';
	$files       = blue_nova_phase_two_v1_3_install_theme_files( $backup_root );
	$woo_state   = blue_nova_phase_two_v1_3_activate_woocommerce();
	$products    = blue_nova_phase_two_v1_3_isolate_legacy_products( $backup_root );
	$pages       = blue_nova_phase_two_v1_3_configure_core( $backup_root );

	flush_rewrite_rules();
	update_option(
		'blue_nova_phase_two_staging_patch_v1_3',
		array(
...
register_activation_hook( __FILE__, 'blue_nova_phase_two_install_v1_3' );

/** Leave an auditable, non-sensitive completion notice for administrators. */
function blue_nova_phase_two_v1_3_admin_notice() {
	$record = get_option( 'blue_nova_phase_two_staging_patch_v1_3' );
	if ( ! $record || ! current_user_can( 'manage_options' ) ) {
		return;
	}
	?>
	<div class="notice notice-success is-dismissible"><p><strong>Phase Two staging v1.3 installed.</strong> WooCommerce core is active; legacy products are preserved as drafts; approved Systems 03–07 are installed; payment gateways and external connections remain off.</p></div>
	<?php
}
add_action( 'admin_notices', 'blue_nova_phase_two_v1_3_admin_notice' );
