<?php
/**
 * Plugin Name: Blue Nova Phase Two Staging Installer v1.6
 * Description: Aligns the staging header, ecommerce utility row, and responsive footer exactly to approved System 01 v1.5 while preserving approved Systems 02-07.
 * Version: 1.6.0
...
function blue_nova_phase_two_v1_6_install_theme_files( $backup_root ) {
...
function blue_nova_phase_two_v1_6_activate_woocommerce() {
...
function blue_nova_phase_two_v1_6_isolate_legacy_products( $backup_root ) {
	global $wpdb;
...
function blue_nova_phase_two_v1_6_disable_gateways() {
...
function blue_nova_phase_two_v1_6_configure_core( $backup_root ) {
	$options = array(
	...
	blue_nova_phase_two_v1_6_disable_gateways();
...
function blue_nova_phase_two_install_v1_6() {
	...
	if ( get_option( 'blue_nova_phase_two_staging_patch_v1_6' ) ) {
		return;
	}

	$backup_root = trailingslashit( WP_CONTENT_DIR ) . 'uploads/blue-nova-phase-two-backups/v1.6-' . gmdate( 'Ymd-His' ) . '/';
	$files       = blue_nova_phase_two_v1_6_install_theme_files( $backup_root );
	$woo_state   = blue_nova_phase_two_v1_6_activate_woocommerce();
	$products    = blue_nova_phase_two_v1_6_isolate_legacy_products( $backup_root );
	$pages       = blue_nova_phase_two_v1_6_configure_core( $backup_root );

	flush_rewrite_rules();
	update_option(
		'blue_nova_phase_two_staging_patch_v1_6',
		array(
...
register_activation_hook( __FILE__, 'blue_nova_phase_two_install_v1_6' );

/** Leave an auditable, non-sensitive completion notice for administrators. */
function blue_nova_phase_two_v1_6_admin_notice() {
	$record = get_option( 'blue_nova_phase_two_staging_patch_v1_6' );
	if ( ! $record || ! current_user_can( 'manage_options' ) ) {
		return;
	}
	?>
	<div class="notice notice-success is-dismissible"><p><strong>Phase Two staging v1.6 installed.</strong> System 01 now matches the approved sticky two-row header and exact five-column/five-tab footer behavior; payments and external connections remain off.</p></div>
	<?php
}
add_action( 'admin_notices', 'blue_nova_phase_two_v1_6_admin_notice' );
